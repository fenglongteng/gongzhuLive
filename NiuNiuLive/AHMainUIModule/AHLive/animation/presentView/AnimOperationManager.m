    ...
    if ([self.userGigtInfos objectForKey:userID]) {
        ...
        if (_isUp == YES) {
            ...
            if (op.model.giftCount != 0) {
                CGRect windowRect = [self.parentView convertRect:CGRectMake(-self.parentView.frame.size.width / 2, screenHeight - 300,self.parentView.frame.size.width / 2, 40) fromView:[AppDelegate getAppdelegateWindow]];
                op.presentView.frame  = windowRect;
                [self.parentView addSubview:op.presentView];
            ...
            if (op.model.giftCount != 0) {
                CGRect windowRect = [self.parentView convertRect:CGRectMake(-self.parentView.frame.size.width / 2, screenHeight - 350,self.parentView.frame.size.width / 2, 40) fromView:[AppDelegate getAppdelegateWindow]];
                op.presentView.frame  = windowRect;
                [self.parentView addSubview:op.presentView];
                op.presentView.originFrame = op.presentView.frame;
