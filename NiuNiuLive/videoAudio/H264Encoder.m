...
//        VTSessionSetProperty(EncodingSession, kVTCompressionPropertyKey_MaxKeyFrameInterval, width);
...
- (void) initEncode:(int)width  height:(int)height
{
    dispatch_sync(aQueue, ^{
        ...
        //设置编码码率，默认不设置的话，导致编码出来的视频很模糊
        SInt32 bitRate = width*height*10;  //越高效果越屌  帧数据越大
...
//        CFRelease(ref);
        status  = VTSessionSetProperty(EncodingSession, kVTCompressionPropertyKey_AverageBitRate, (__bridge CFTypeRef)@(bitRate)); // bps
        status += VTSessionSetProperty(EncodingSession, kVTCompressionPropertyKey_DataRateLimits, (__bridge CFArrayRef)@[@(bitRate*2/8), @1]); // Bps
        ...
        // Tell the encoder to start encoding 开始编码
        VTCompressionSessionPrepareToEncodeFrames(EncodingSession);
    });
}
