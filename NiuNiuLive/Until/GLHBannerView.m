...
#import "Masonry.h"
#define UIImageViewTagBegin 100
...
-(void)showImageRotatorComponentWithImageArray:(NSArray *)imageUrlArray{
    _pageCtrl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, SCREEN_WIDTH, SCREEN_WIDTH, 20)];
    _pageCtrl.currentPageIndicatorTintColor = [UIColor whiteColor];
    _pageCtrl.pageIndicatorTintColor = AHColor(255, 255, 255, 0.5);
    _pageCtrl.tag = 200;
    [_headerView addSubview:_pageCtrl];
    [self setBannerImageArray:imageUrlArray];
}

-(void)customTableHeaderView:(UIView*)customHeaderView{
    _customView = customHeaderView;
    _customView.tag = 300;
    [self showImageRotatorComponentWithImageArray:@[@"1111"]];
}

- (void)setBannerImageArray:(NSArray *)imageUrlArray
{
   
    //图片链接数组
    _imageUrlArray = [imageUrlArray mutableCopy];
    ...
    [_pageCtrl mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(_bannerScrollView).offset(-10);
    }];
    UIView * container ;
    if (_customView) {
        container = _customView;
    }else{
    ...
    if(_imageUrlArray.count)
    {
        ...
        for (int i = 0; i<_imageUrlArray.count; i++)
        {
            ...
            bannerImageView.tag = UIImageViewTagBegin + i;
            UITapGestureRecognizer *tapGR = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickeImage:)];
            ...
            if (_customView) {
                ...
                bannerImageView.hidden = YES;
                    ...
                    bannerImageView.frame = CGRectMake(SCREEN_WIDTH*i+15, 0, SCREEN_WIDTH, self.height);
            }
            lastView = bannerImageView;
            NSString *imageURL = _imageUrlArray[i];
            if (i<pathArray.count) {
                UIImage *image = [UIImage imageNamed:pathArray[i]];
                [bannerImageView sd_setImageWithURL:[NSURL URLWithString:imageURL]placeholderImage:image];
            }else{
                  UIImage *image = [UIImage imageNamed:pathArray[i%3]];
                [bannerImageView sd_setImageWithURL:[NSURL URLWithString:imageURL]placeholderImage:image];
            }
...
-(void)clickeImage:(UITapGestureRecognizer*)sender{
    UIView *view = sender.view;
    if ([self.clickImageDelegate respondsToSelector:@selector(clickOnTheImageOfIndex:)]) {
        [self.clickImageDelegate clickOnTheImageOfIndex:view.tag - UIImageViewTagBegin];
    }
}
...
-(void)setOffSetY:(CGFloat)offSetY{
     ...
     //  CGFloat X = offSetY;
        CGFloat Y = offSetY;
    if ([_headerView viewWithTag:200]) {
      //  X =  _index * offSetY;
        if (offSetY == 0) {
