...
- (void)viewDidLoad {
    ...
    if(self.showCustomView){
        //头部视图
        //        self.headView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 50.f)];
        //        self.headView.backgroundColor = [UIColor redColor];
        //        [self.view addSubview:self.headView];
        
        //关闭按钮
        self.doneBtn = [UIButton buttonWithType:UIButtonTypeSystem];
        self.doneBtn.frame = CGRectMake(self.view.frame.size.width -16 -30, 32, 35, 35);
        [self.doneBtn setImage:[UIImage imageNamed:@"btn_zb_close"] forState:UIControlStateNormal];
        [self.doneBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        self.doneBtn.titleLabel.font = [UIFont systemFontOfSize:18];
        [self.view addSubview:self.doneBtn];
        [self.doneBtn addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        
        //当前位置的是第几张图片的label
        _currentIndexLabel = [[UILabel alloc]initWithFrame:CGRectMake(16, 32, 50, 21)];
        ...
        [self setUpCurrentIndexLabel];
        [_currentIndexLabel sizeToFit];
        
        [self.doneBtn setCenterY: self.currentIndexLabel.centerY];
        //底部视图
        if(_isOwn){
            self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height- 105, [UIScreen mainScreen].bounds.size.width, 105)];
            //设置头像button
            self.setHeadImageBt = [UIButton buttonWithType:UIButtonTypeCustom];
            self.setHeadImageBt.frame = CGRectMake(15, 15, self.bottomView.frame.size.width - 30, 30);
            [self.setHeadImageBt setTitle:@"设置头像" forState:UIControlStateNormal];
            [self.setHeadImageBt addBorderColor:[UIColor blackColor] andwidth:1 andCornerRadius:5];
            [self.bottomView addSubview:self.setHeadImageBt];
            [self.setHeadImageBt setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            self.setHeadImageBt.titleLabel.font = [UIFont systemFontOfSize:17];
            [self.setHeadImageBt addTarget:self action:@selector(bt_setHead:) forControlEvents:UIControlEventTouchUpInside];
            //删除照片按钮
            self.deleteBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            self.deleteBtn.frame = CGRectMake(15, 60, self.bottomView.frame.size.width - 30, 30);
            self.deleteBtn.layer.borderWidth = 1;
            [self.deleteBtn setTitle:@"删除照片" forState:UIControlStateNormal];
            [self.deleteBtn addBorderColor:[UIColor blackColor] andwidth:1 andCornerRadius:5];
            [self.bottomView addSubview:self.deleteBtn];
            [self.deleteBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
            [self.deleteBtn addTarget:self action:@selector(bt_deletePhoto:) forControlEvents:UIControlEventTouchUpInside];
            self.setHeadImageBt.titleLabel.font = [UIFont systemFontOfSize:17];
        }else{
            self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height- 65, [UIScreen mainScreen].bounds.size.width, 65)];
            self.setAttentionView = [[AHSetAttentionOfPhotoAlbum alloc]initWithFrame:CGRectMake(0, 0, screenWidth, 65)];
            [self.setAttentionView.attentionBt addTarget:self action:@selector(bt_attention:) forControlEvents:UIControlEventTouchUpInside];
            [self.bottomView addSubview:self.setAttentionView];
        }
        ...
        //图片保存到系统相册按钮
        //        self.saveSystemPhoto = [UIButton buttonWithType:UIButtonTypeCustom];
        //        self.saveSystemPhoto.frame = CGRectMake(CGRectGetMinX(self.deleteBtn.frame) - 50, 0, 50, 50);
        //        self.saveSystemPhoto.layer.borderWidth = 1;
        //        [self.saveSystemPhoto setTitle:@"保存" forState:UIControlStateNormal];
        //        [self.bottomView addSubview:self.saveSystemPhoto];
    }else{
        // Toolbar
    ...
    if (self.showPageControl && [self numberOfPhotos] > 1)
    {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 70, self.view.frame.size.width, 20)];
...
#pragma mark 设置头像 删除照片 关注

//设置头像
-(void)bt_setHead:(UIButton*)sender{
    ...
    NSMutableString *imageString = [photo.photoURL.absoluteString mutableCopy];
    NSRange range = [imageString rangeOfString:[AHPersonInfoManager manager].getInfoModel.webApiUserGetAvatarURL];
    [imageString deleteCharactersInRange:range];
       editeInfoRequst.avatar = imageString;
    ...
    [[AHTcpApi shareInstance]requsetMessage:editeInfoRequst classSite:@"Users" completion:^(id response, NSString *error) {
        ...
        if (editeRespose.result == 0 ) {
            ...
            infoModel.avatarURL = imageString;
...
-(void)showAlertWithMessage:(NSString*)messageString{
    UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"温馨提示" message:messageString delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
    [alertView show];
}

//删除照片
-(void)bt_deletePhoto:(UIButton*)sender{
    MWPhoto *photo = (MWPhoto *)[self photoAtIndex:_currentPageIndex];
    NSMutableString *imageString = [photo.photoURL.absoluteString mutableCopy];
    NSRange range = [imageString rangeOfString:[AHPersonInfoManager manager].getInfoModel.webApiUserGetAvatarURL];
    [imageString deleteCharactersInRange:range];
    ...
    if (self.deletePhotoBlock) {
        if (_deletePhotoBlock(imageString)) {
...
//关注
-(void)bt_attention:(UIButton*)sender{
    AHPersonInfoModel *infoModel =  _setAttentionView.infoModel;
    static BOOL isAttention;
    isAttention = !isAttention;
    if (isAttention) {
        [self requestAttentWithUserId:infoModel.userId button:sender];
    }else{
        [self requestUnAttentWithUserId:infoModel.userId button:sender];
    }
}

-(void)requestAttentWithUserId:(NSString*)userId button:(UIButton*)button{
    
    UsersLikeUserRequest *likeUserRequest = [[UsersLikeUserRequest alloc]init];
    
    GPBStringBoolDictionary *boolDic =  [GPBStringBoolDictionary dictionaryWithBool:NO forKey:userId];
    ...
    [[AHTcpApi shareInstance]requsetMessage:likeUserRequest classSite:UsersClassName completion:^(id response, NSString *error) {
        ...
        if (liskeUserRespose.result != 0) {
            ...
            [button setTitle:@"已经关注" forState:UIControlStateNormal];
            button.backgroundColor = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.3];
            
        }
    }];
}

-(void)requestUnAttentWithUserId:(NSString*)userId button:(UIButton*)button{
    UsersUnlikeUserRequest  *unlikeUserRequest = [[UsersUnlikeUserRequest alloc]init];
    unlikeUserRequest.userId = userId;
    [[AHTcpApi shareInstance]requsetMessage:unlikeUserRequest classSite:UsersClassName completion:^(id response, NSString *error) {
        ...
        if (unlikeRespose.result != 0) {
            ...
            [button setBackgroundColor:UIColorFromRGB(0xfed215)];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.dismissWithTap) {
        [self setControlsHidden:YES animated:NO permanent:NO]; //显示或隐藏标记
    } else {
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self setControlsHidden:NO animated:NO permanent:YES]; //显示或隐藏标记
...
- (void)setNavBarAppearance:(BOOL)animated {
    [self.navigationController setNavigationBarHidden:YES animated:animated]; //修改过BarHidden的值原来为NO
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
    ...
    if ([self areControlsHidden]) {
        self.navigationController.navigationBarHidden = YES; //修改过BarHidden的值原来为NO
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (![self numberOfPhotos]) {
        // Show controls
        [self setControlsHidden:NO animated:YES permanent:YES]; //显示或隐藏标记
        return;
    }
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    [self setControlsHidden:YES animated:YES permanent:NO]; //显示或隐藏标记
}
...
- (void)hideGrid {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self setControlsHidden:NO animated:YES permanent:NO]; // retrigger timer //显示或隐藏标记
    }];
...
- (void)showControls {
    ...
    [self setControlsHidden:NO animated:YES permanent:NO];//显示或隐藏标记
}
...
- (void)toggleControls {
    if (self.dismissWithTap) {
        ...
        [self setControlsHidden:![self areControlsHidden] animated:YES permanent:NO];//显示或隐藏标记
    }
}
...
//设置图片所在位置label
-(void)setUpCurrentIndexLabel{
    if (_currentIndexLabel) {
        _currentIndexLabel.text = [NSString stringWithFormat:@"%ld/%ld",_currentPageIndex+1,self.numberOfPhotos];
        [_currentIndexLabel sizeToFit];
    }
}
